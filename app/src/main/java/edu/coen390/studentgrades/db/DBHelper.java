...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private static final String CREATE_COURSE_TABLE = "CREATE TABLE " + Config.TABLE_NAME_COURSE + " ("
    ...
    private static final String CREATE_ASSIGNMENT_TABLE = "CREATE TABLE " + Config.TABLE_NAME_ASSIGNMENT + " ("
    ...
    public int updateCourse(Course course) {
        ...
        // updating row
        return db.update(Config.TABLE_NAME_COURSE, values, Config.COLUMN_COURSE_ID + " = ?",
    ...
    public long createAssignment(Assignment assignment){
        ...
        try{
            id = db.insert(Config.TABLE_NAME_ASSIGNMENT,null,assignmentContentValues);
        }catch (SQLException e){
            Log.d(TAG, "Exception: " + e.getMessage());
            Toast.makeText(context,"Operation Failed: " + e.getMessage(), Toast.LENGTH_LONG).show();
        } finally {
        ...
        try {
            ...
            if(cursor != null){
                if(cursor.moveToPosition(pos)){
                    ...
                    long a_id       = cursor.getInt(cursor.getColumnIndex(Config.COLUMN_ASSIGNMENT_ID));
                    long c_id       = cursor.getInt(cursor.getColumnIndex(Config.COLUMN_COURSE_ID));
                    String title = cursor.getString(cursor.getColumnIndex(Config.COLUMN_ASSIGNMENT_TITLE));
                    double grade  = cursor.getDouble(cursor.getColumnIndex(Config.COLUMN_ASSIGNMENT_GRADE));
                ...
                if(cursor.moveToFirst()){
                    ...
                    do{
                        if(cursor.getInt(cursor.getColumnIndex(Config.COLUMN_COURSE_ID)) == c_id){
                            long a_id       = cursor.getInt(cursor.getColumnIndex(Config.COLUMN_ASSIGNMENT_ID));
                            long course_id       = cursor.getInt(cursor.getColumnIndex(Config.COLUMN_COURSE_ID));
                            String title = cursor.getString(cursor.getColumnIndex(Config.COLUMN_ASSIGNMENT_TITLE));
                            double grade  = cursor.getDouble(cursor.getColumnIndex(Config.COLUMN_ASSIGNMENT_GRADE));
    ...
    public int updateAssignment(Assignment assignment) {
        ...
        // updating row
        return db.update(Config.TABLE_NAME_ASSIGNMENT, values, Config.COLUMN_ASSIGNMENT_ID + " = ?",
    ...
    public void deleteAssignments(long c_id) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(Config.TABLE_NAME_ASSIGNMENT, Config.COLUMN_COURSE_ID + " = ?",
                new String[] { String.valueOf(c_id) });
    }

    public double getAssignmentsAvg(long c_id){
        SQLiteDatabase db = this.getReadableDatabase();
        double sum = 0.0;
        double count= 0.0;
